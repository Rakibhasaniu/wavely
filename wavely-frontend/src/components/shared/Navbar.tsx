            ...
            <div id="_prfoile_drop" className={`_nav_profile_dropdown _profile_dropdown${profileOpen ? ' show' : ''}`}>
              <div className="_nav_profile_dropdown_info">
                <div className="_nav_profile_dropdown_image">
                  {user?.avatar ? (
                    <img src={user.avatar} alt={fullName} className="_nav_drop_img" />
                  ) : (
                    <div className="_nav_drop_img rounded-circle d-flex align-items-center justify-content-center bg-primary text-white fw-bold" style={{ width: 44, height: 44, fontSize: 16 }}>
                      {user?.firstName?.charAt(0).toUpperCase()}
                    </div>
                  )}
                </div>
